...
public class TouchInputListener implements View.OnTouchListener {
    ...
    public TouchInputListener(MainView view) {
        super();
        this.mView = view;
    }
            ...
            case MotionEvent.ACTION_UP:
                ...
                if (!hasMoved) {
                    if (iconPressed(mView.sXNewGame, mView.sYIcons)) {
                        ...
                        if( mView.game.gameState==MainGame.GAME_LOST||mView.game.gameState==MainGame.GAME_WIN){
                            mView.game.newGame();
                            return true;
                        }
                        Cell cell = null;
                        for (int xx = 0; xx < mView.game.numSquaresX; xx++) {
                            for (int yy = 0; yy <  mView.game.numSquaresY; yy++) {
                                cell = mView.cellField[xx][yy];
                                if(inRange(cell.getStartingX(), x, cell.getEndingX())
                                        && inRange(cell.getStartingY(), y, cell.getEndingY())){
                                    try{
                                        String value_str = mView.game.grid.field[cell.getY()][cell.getX()].getValue()+"";
                                    }catch (Exception e){
                                        Log.i("error : ", e.getMessage());
                                        mView.game.userAddTile(new Tile(cell.getY(), cell.getX(),4,MainGame.TILE_TITLE_X));
                                    }
